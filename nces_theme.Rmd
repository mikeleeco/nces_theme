...
gg <- ggplot(tf, aes(x = Year, y = Value, frame = Year)) + geom_line(aes(group=Race, color=Race, cumulative = TRUE), size=1.25) + scale_x_date(labels=year(unique(res$Year)), breaks=unique(res$Year)) 
...
gg_animate(gg,"line_tween.gif", interval = .05,pause = .05, ani.width = 1200, ani.height = 650, title_frame = FALSE)
```